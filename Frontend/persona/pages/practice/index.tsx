...
import { clickedEmotionState, clickedGenreState, clickedBtnState, optionState, keywordState, sortingState, pageState} from '../../states/practiceFilterState';
      ...
      <div className={style.container}>
        ...
        <div className={style.script}>
          <div className={style.sorting}>
            <div onClick={handler1}>최신순</div>            
            |
            <div onClick={handler2}>인기순</div>
            |
            <div onClick={handler3}>참여순</div>            
            |
            <div onClick={handler4}>조회순</div>            
          </div>
...
export async function getServerSideProps() {
  const clickedEmotion = useRecoilValue(clickedEmotionState);
  const clickedGenre = useRecoilValue(clickedGenreState);
  const clickedKeyword = useRecoilValue(keywordState);
  const clickedOption = useRecoilValue(optionState);
  const clickedSort = useRecoilValue(sortingState);
